...
func buildOnParentChain(
	...
	chainTestClient.ConsensusNode, err = arbnode.CreateNodeFullExecutionClient(
		...
		addresses, validatorTxOptsPtr, sequencerTxOptsPtr, dataSigner, nil, nil, fatalErrChan, parentChainId, nil, locator.LatestWasmModuleRoot(), nil)
	Require(t, err)
...
func (b *NodeBuilder) BuildL2(t *testing.T) func() {
	...
	b.L2.ConsensusNode, err = arbnode.CreateNodeFullExecutionClient(
		...
		nil, nil, nil, nil, nil, nil, nil, fatalErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), nil)
	Require(t, err)
...
func (b *NodeBuilder) BuildEspressoCaffNode(t *testing.T, existing *NodeBuilder, withSnapshotSigner bool) (func(), error) {
	...
	if withSnapshotSigner {
		...
		caffNodeTxopts := existing.L1Info.GetDefaultTransactOpts("Sequencer", context.Background())
		b.L2.ConsensusNode, err = arbnode.CreateNodeFullExecutionClient(
			b.ctx, b.L2.Stack, execNode, execNode, execNode, execNode, arbDb, NewFetcherFromConfig(b.nodeConfig), blockchain.Config(),
			l1Client, deployInfo, nil, nil, nil, &snapshotSignerAddress, snapshotSigner, fatalErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), &caffNodeTxopts)
		Require(t, err)
	} else {
		b.L2.ConsensusNode, err = arbnode.CreateNodeFullExecutionClient(
			b.ctx, b.L2.Stack, execNode, execNode, execNode, execNode, arbDb, NewFetcherFromConfig(b.nodeConfig), blockchain.Config(),
			l1Client, deployInfo, nil, nil, nil, nil, nil, fatalErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), nil)
		Require(t, err)
	}
...
func (b *NodeBuilder) RestartCaffNode(t *testing.T, withSnapshotSigner bool) {
	...
	if withSnapshotSigner {
		...
		caffNodeTxOpts := b.L1Info.GetDefaultTransactOpts("Sequencer", b.ctx)
		currentNode, err = arbnode.CreateNodeFullExecutionClient(b.ctx, stack, execNode, execNode, execNode, execNode, arbDb, NewFetcherFromConfig(b.nodeConfig), blockchain.Config(), b.L1.Client, b.addresses, nil, nil, nil, &signerAddress, snapshotSigner, feedErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), &caffNodeTxOpts)
		Require(t, err)
	} else {
		currentNode, err = arbnode.CreateNodeFullExecutionClient(b.ctx, stack, execNode, execNode, execNode, execNode, arbDb, NewFetcherFromConfig(b.nodeConfig), blockchain.Config(), b.L1.Client, b.addresses, nil, nil, nil, nil, nil, feedErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), nil)
		Require(t, err)
	}
...
func Create2ndNodeWithConfig(
	...
	if useExecutionClientOnly {
		currentNode, err = arbnode.CreateNodeExecutionClient(ctx, chainStack, currentExec, arbDb, NewFetcherFromConfig(nodeConfig), blockchain.Config(), parentChainClient, addresses, &validatorTxOpts, &sequencerTxOpts, dataSigner, nil, nil, feedErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), nil)
	} else {

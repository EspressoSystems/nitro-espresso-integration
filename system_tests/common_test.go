...
func (b *NodeBuilder) BuildEspressoCaffNode(t *testing.T, existing *NodeBuilder, withSnapshotSigner bool) (func(), error) {
	...
	if withSnapshotSigner {
		snapshotSignerKey := b.L1Info.GetInfoWithPrivKey("Sequencer").PrivateKey
		snapshotSigner := signature.DataSignerFromPrivateKey(snapshotSignerKey)
		txOpts := existing.L1Info.GetDefaultTransactOpts("Sequencer", existing.ctx)
		teeAddress := existing.L1Info.GetAddress("Sequencer")
		faucetAddress := existing.L1Info.GetAddress("Faucet")
		teeBal := existing.L1.GetBalance(t, teeAddress)
		faucetBal := existing.L1.GetBalance(t, faucetAddress)

		log.Info("BALANCES", "teeBalance (Sequencer)", teeBal, "faucetBalance", faucetBal)
		...
		b.L2.ConsensusNode, err = arbnode.CreateNodeFullExecutionClient(
			...
			l1Client, deployInfo, nil, nil, nil, &teeAddress, fatalErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), &txOpts, snapshotSigner)
		Require(t, err)
	} else {
...
func (b *NodeBuilder) RestartCaffNode(t *testing.T, withSnapshotSigner bool) {
	...
	if withSnapshotSigner {
		...
		caffDB, err := authdb.NewAuthDB(chainDb, teeHMAC)
		Require(t, err)
		currentNode, err = arbnode.CreateNodeFullExecutionClient(b.ctx, stack, execNode, execNode, execNode, execNode, arbDb, &caffDB, NewFetcherFromConfig(b.nodeConfig), blockchain.Config(), b.L1.Client, b.addresses, nil, nil, nil, &signerAddress, feedErrChan, big.NewInt(1337), nil, locator.LatestWasmModuleRoot(), nil, nil)
		Require(t, err)
	} else {
		caffDB, err := authdb.NewAuthDB(chainDb, nil)
		Require(t, err)

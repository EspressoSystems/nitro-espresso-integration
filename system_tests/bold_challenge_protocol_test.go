...
func createTestNodeOnL1ForBoldProtocol(
	...
	currentNode, err = arbnode.CreateNodeFullExecutionClient(
		...
		addresses, sequencerTxOptsPtr, sequencerTxOptsPtr, dataSigner, nil, fatalErrChan, parentChainId,
		nil, // Blob reader.
		locator.LatestWasmModuleRoot(),
		nil,
	)
...
func create2ndNodeWithConfigForBoldProtocol(
	...
	locator, err := server_common.NewMachineLocator("")
	Require(t, err)
	l2node, err := arbnode.CreateNodeFullExecutionClient(ctx, l2stack, execNode, execNode, execNode, execNode, l2arbDb, nil, NewFetcherFromConfig(nodeConfig), l2blockchain.Config(), l1client, addresses, &txOpts, &txOpts, dataSigner, nil, fatalErrChan, l1ChainId, nil /* blob reader */, locator.LatestWasmModuleRoot(), nil)
	Require(t, err)

...
func testSequencerInboxReaderImpl(t *testing.T, validator bool) {
	...
	for i := 1; i < seqInboxTestIters; i++ {
		if i%10 == 0 {
			...
			if i%5 == 0 {
				tx, err = seqInbox.AddSequencerL2Batch99020501(&seqOpts, big.NewInt(int64(len(blockStates))), batchData, big.NewInt(1), gasRefunderAddr, big.NewInt(0), big.NewInt(0), createDummyHotShotHeightAndSignature(t))
			} else {
				tx, err = seqInbox.AddSequencerL2BatchFromOrigin37501551(&seqOpts, big.NewInt(int64(len(blockStates))), batchData, big.NewInt(1), gasRefunderAddr, common.Big0, common.Big0, createDummyHotShotHeightAndSignature(t))
			}
			Require(t, err)

...
type BatcherAddrMonitor struct {
	...
	// results[0]: batcher addresses valid from eventUpdatesAt[0] to eventUpdatesAt[1]
	// results[i]: batcher addresses valid from eventUpdatesAt[i] to eventUpdatesAt[i+1]
	// results[last]: batcher addresses valid from eventUpdatesAt[last] to current height (inclusive)
	// Cache all the results to avoid repeated lookups. Since the batcher addresses are relatively stable,
...
func (b *BatcherAddrMonitor) IsValid(ctx context.Context, batcherAddress common.Address, l1Height uint64) (bool, error) {
	if len(b.eventUpdatesAt) == 0 {
		b.addEventUpdates([]uint64{b.deployAt})
	}
	height := b.deployAt
...
func (b *BatcherAddrMonitor) Store() error {
	...
	err := authdb.WriteAddresses(newBatch, b.results)
	if err != nil {
...
func (b *BatcherAddrMonitor) addEventUpdates(updates []uint64) {
	...
	for i := 0; i < len(updates); i++ {
		b.results = append(b.results, make(map[common.Address]bool))
	}
}

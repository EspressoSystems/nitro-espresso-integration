...
// In the case where there is a proxy setup, connect to that first
func (bc *BroadcastClient) connectWithProxy(ctx context.Context, proxyURL *url.URL, addr string) (net.Conn, error) {
	// Connect to proxy
	var conn net.Conn
	var err error
...
func (bc *BroadcastClient) connect(ctx context.Context, nextSeqNum arbutil.MessageIndex) (io.Reader, error) {
	...
	timeoutDialer := ws.Dialer{
		...
		NetDial: func(ctx context.Context, network, addr string) (net.Conn, error) {
			...
			return bc.connectWithProxy(ctx, proxyURL, addr)
		},
	}

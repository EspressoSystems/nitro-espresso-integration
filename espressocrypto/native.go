...
/*
#cgo CFLAGS: -I./lib/espresso-crypto-helper/target/release
#cgo LDFLAGS: -L./lib/espresso-crypto-helper/target/release -lespresso_crypto_helper
#include <stdbool.h>
#include <stdint.h>

bool verify_merkle_proof_helper(
    const uint8_t* proof_ptr, uintptr_t proof_len,
    const uint8_t* header_ptr, uintptr_t header_len,
    const uint8_t* block_comm_ptr, uintptr_t block_comm_len,
    const uint8_t* circuit_block_ptr, uintptr_t circuit_block_len
);
...
func verifyMerkleProof(proof []byte, header []byte, blockComm []byte, circuitBlock []byte) bool {
	...
	headerLen := C.uintptr_t(len(header))

	blockCommPtr := (*C.uint8_t)(unsafe.Pointer(&blockComm[0]))
	blockCommLen := C.uintptr_t(len(blockComm))

	circuitBlockPtr := (*C.uint8_t)(unsafe.Pointer(&circuitBlock[0]))
	circuitBlockLen := C.uintptr_t(len(circuitBlock))

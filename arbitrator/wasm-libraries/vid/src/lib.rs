...
pub unsafe extern "C" fn go__github_com_offchainlabs_nitro_arbvid_verifyNamespace(sp: GoStack) {
    verify_namespace_helper(sp);
}

pub unsafe extern "C" fn verify_namespace_helper(sp: GoStack) {
    //(namespace int, proofBuf []byte, blockCommBuf []byte, txsBuf []byte)
    ...
    let txs_bytes = read_slice(txs_buf_ptr, txs_buf_len);

...
func mainImpl() int {
	...
	nodeConfig.Node.Espresso.EspressoCaffNode.ResolveDirectoryNames(nodeConfig.Persistent.Chain)

	var espressoCaffNodeInitArgs *arbnode.EspressoCaffNodeInitArgs

	if nodeConfig.Node.Espresso.EspressoCaffNode.Enable && nodeConfig.Node.Espresso.EspressoCaffNode.EspressoTeeType != "" {
		caffNodePrivateKey, err := espresso_tee_utils.ReadEnclavePrivateKey(nodeConfig.Node.Espresso.EspressoCaffNode.KeyPairAttestationsPath, nodeConfig.Chain.ID)
		if err != nil {
			flag.Usage()
			log.Crit("error reading enclave private key for Espresso Caff node", "path", nodeConfig.Node.Espresso.EspressoCaffNode.KeyPairAttestationsPath, "err", err)
		}

		teeHMAC, err = espresso_tee_utils.DeriveHmac(nodeConfig.Node.Espresso.EspressoCaffNode.KeyPairAttestationsPath, nodeConfig.Chain.ID)
		if err != nil {
			flag.Usage()
	...
	if nodeConfig.Node.Espresso.EspressoCaffNode.Enable && nodeConfig.Node.Espresso.EspressoCaffNode.SnapshotChecksum != "" {
		// Check that TEE is enabled
		if nodeConfig.Node.Espresso.EspressoCaffNode.EspressoTeeType == "" {
			log.Error("snapshot verification requires TEE, but no TEE type was specified")
			return 1
		}
		log.Info("Verifying the snapshot", "snapshot checksum", nodeConfig.Node.Espresso.EspressoCaffNode.SnapshotChecksum)
		initializeCaffNodeTags, err = arbutil.VerifySnapshot(nodeConfig.Node.Espresso.EspressoCaffNode.SnapshotChecksum, stack.InstanceDir(), stack.ResolvePath("l2chaindata"), stack.ResolveAncient("l2chaindata", nodeConfig.Persistent.Ancient), espressoCaffNodeInitArgs.CaffNodePrivateKey)
		if err != nil {

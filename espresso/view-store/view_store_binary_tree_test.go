...
func TestEspressoViewStoreInsert(t *testing.T) {
	...
	t.Run("Insert a view which has the same view number but lower builder commitment", func(t *testing.T) {
		...
		root = Insert(root, 5, "BUILDER_COMMITMENT~tEvs0rxqOiMCvfe2R0omNNaphSlUiEDrb2q0IZpRcfA_", mkHash("4"))
		...
		if viewStoreFor2ViewNumber.Right.View.builderCommitment != "BUILDER_COMMITMENT~tEvs0rxqOiMCvfe2R0omNNaphSlUiEDrb2q0IZpRcfA_" {

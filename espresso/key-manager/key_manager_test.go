...
func TestEspressoKeyManager(t *testing.T) {
	...
	t.Run("SGX NewEspressoKeyManager", func(t *testing.T) {
		...
		km := espresso_key_manager.NewEspressoKeyManager(mockEspressoTEEVerifierClient, mockEspressoNitroTEEVerifier, dataposter, dataSigner, espresso_key_manager.SGX, espressotee.Test, nil, "", "", "")
	...
	t.Run("Nitro Registry", func(t *testing.T) {
		...
		mockEspressoTEEVerifierClient.On("RegisteredServices", mock.Anything, mock.Anything, mock.Anything).Return(false, nil).Once()
		mockEspressoTEEVerifierClient.On("RegisteredServices", mock.Anything, mock.Anything, mock.Anything).Return(true, nil).Once()
		km := espresso_key_manager.NewEspressoKeyManager(mockEspressoTEEVerifierClient, mockEspressoNitroTEEVerifier, dataposter, dataSigner, espresso_key_manager.NITRO, espressotee.Test, nil, "", "", "http:/127.0.0.1")
		registered := km.HasRegistered()
		assert.False(t, registered, "Should start unregistered")

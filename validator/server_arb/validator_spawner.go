...
func (v *ArbitratorSpawner) loadEntryToMachine(ctx context.Context, entry *validator.ValidationInput, mach *ArbitratorMachine) error {
	...
	log.Info("setting hotshot commitment", "height", entry.BlockHeight, "commitment", entry.HotShotCommitment[:])
	...
	if err != nil {
		...
		return fmt.Errorf("error while setting hotshot liveness: %w", err)
	}
	return nil
}

...
if $BUILD_WASM; then
    ...
    cmake ../../ -DCMAKE_C_COMPILER=emcc -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS=-fPIC -DCMAKE_INSTALL_PREFIX="$TEMP_INSTALL_DIR_ABS" -DCMAKE_AR="$(which emar)" -DCMAKE_RANLIB="$(which touch)" -DCMAKE_POLICY_VERSION_MINIMUM=3.5
    make -j
    make install
...
if $BUILD_LOCAL; then
    ...
    cmake ../../ -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="$TARGET_DIR_ABS" -DCMAKE_POLICY_VERSION_MINIMUM=3.5
    make -j
    make install
    cd ..
fi
